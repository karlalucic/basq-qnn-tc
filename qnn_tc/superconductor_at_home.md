...
- Classical inference is deterministic and reproducible, so the server re-runs
  a random sample of each returned work unit and checks a result hash; errors
  and cheating are caught for free.
- There is no privacy problem: the model and candidate generator are public; it
  is a public-good screen.
...
uncertainty of the rest**, so the model is uncertain *exactly* where it predicts a
...
- The funnel re-discovers the champion chemistry: with no labels, the top of the
...
## Honest limits
- The screen is bounded by predictor reliability: it is only as good as the
  model, and the model is weakest out-of-distribution, which is where discovery
  lives. Uncertainty filtering *manages* this; it does not remove it.
